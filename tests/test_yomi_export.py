...
class YomiExportTests(unittest.TestCase):
    ...
    def test_export_debug_comparison_texts_uses_hybrid_jsonl_and_writes_debug_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "yomi_corpus.yomi.export.generate_mechanical_yomi"
            ) as mocked_generator:
                mocked_generator.side_effect = [
                    MechanicalYomi(rendered="A/エー", certain=False),
                    MechanicalYomi(rendered="B/ビー", certain=False),
                ]
            ...
            self.assertTrue(
                all(
                    call.kwargs["strategy_name"] == "sudachi_only_v1"
                    for call in mocked_generator.call_args_list
                )
            )
            self.assertEqual(
